...
// local
// --------------------------------------------------
use crate::archive::CompressedFile;
...
pub(crate) struct DownloadParams {
    /// One-time download token passed as the `k` query parameter
    k: Option<String>,
}
...
impl DownloadItem {
    ...
    ///
    /// Reads the [`ArchiveState`] under a read lock and dispatches based on
    /// the current state:
    /// * [`ArchiveState::Preparing`] - returns [`DownloadError::Preparing`]
    /// * [`ArchiveState::Ready`] - reads the cache file from disk and returns its bytes
    /// * [`ArchiveState::Failed`] - returns [`DownloadError::ArchiveFailed`]
    /// * [`ArchiveState::NotNeeded`] - falls back to building the archive in memory
    ///
    /// Increments and decrements `active_serving` around any disk read to
    /// prevent the cache file from being deleted while in use
    ///
    /// # Arguments
    ///
    /// * `token` - The download token, used only for log messages
    async fn serve_archive(&self, token: &str) -> Result<DownloadResponse, DownloadError> {
        ...
        match &*archive {
            ...
            // used
            // --------------------------------------------------
            ArchiveState::Used => Err(DownloadError::LimitReached),
            // --------------------------------------------------
    ...
    /// Serves a single file directly without creating an archive
    ///
    /// Reads the file at `self.paths[0]` into memory and returns it as an
    /// `application/octet-stream` response. Increments `active_serving` before
    /// the read and decrements it after, whether the read succeeds or fails
    ///
    /// # Safety
    ///
    /// Assumes `self.paths` is non-empty; callers must ensure a single-file
    /// item has at least one path before invoking this method
