...
///
/// Sleeps for [`HEALTH_CHECK_INTERVAL`] between each pass. On each pass:
/// * Finds archives whose cache file was deleted externally and marks them `Failed`
/// * Finds archives stuck in `Preparing` past [`ARCHIVE_TIMEOUT`] and marks them `Failed`
/// * Removes cache files for expired or fully-consumed links
/// * Warns about active links whose source files are missing from disk
/// * Removes orphaned `.tmp` files older than [`TMP_FILE_MAX_AGE`]
///
...
pub(crate) async fn health_check_loop() {
    loop {
        ...
        let stuck: Vec<_> = links
            ...
            .collect();
        // --------------------------------------------------
        // clean up expired/used links that still have a cache file
        // --------------------------------------------------
...
fn cleanup_orphaned_tmp_files() {
    let Ok(entries) = std::fs::read_dir(crate::archive::ARCHIVE_CACHE_DIR) else {
        return;
    };
