...
/// - Download server: Handles file downloads using generated links
///
...
impl Server {
    ...
    async fn init() -> Self {
        ...
        let state = match AppState::load_state(&data_dir) {
            Ok(links) => {
                let count = links.len();
                tracing::info!("Loaded {count} persisted links from {data_dir:?}");
                // --------------------------------------------------
                // re-trigger archive creation for files which are:
                // - multi-file
                // - active (not deleted, expired, or reached max downloads)
                // --------------------------------------------------
                let state = Arc::new(AppState::with_links(links));
                let links_read = state.links.read().await;
                links_read
                    .iter()
                    .filter(|(_, item)| {
                        item.is_multi_file
    ...
    pub async fn run(self) -> std::io::Result<()> {
        ...
        smol::spawn(async move {
            ...
            loop {
                ...
                // --------------------------------------------------
                let links = state_for_health.links.read().await;
    ...
    ///
    /// The download server handles only file download requests using links
