...
impl PersistedDownloadItem {
    ...
    /// # Arguments
    ///
    /// * `item` - The live download item to snapshot
    /// * `now` - The reference instant for computing elapsed and remaining times
    pub(crate) fn from_download_item(item: &DownloadItem, now: std::time::Instant) -> Self {
        // --------------------------------------------------
        // read the current download count atomically
        // --------------------------------------------------
        let count = item
            .download_count
            .load(std::sync::atomic::Ordering::Relaxed);
        // --------------------------------------------------
        // compute seconds remaining until expiry, or None if
        // the link does not expire or has already expired
        // --------------------------------------------------
        ...
        // --------------------------------------------------
        // compute how many seconds ago this item was created
        // --------------------------------------------------
        let created_ago_seconds = now.duration_since(item.created_at).as_secs();
        // --------------------------------------------------
        // record the wall-clock time of this snapshot as a
        // unix timestamp for auditing purposes
        // --------------------------------------------------
        let saved_at = std::time::SystemTime::now()
            ...
            .as_secs();
        // --------------------------------------------------
        // construct and return the serializable snapshot
        // --------------------------------------------------
        Self {
